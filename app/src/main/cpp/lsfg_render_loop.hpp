...
// True when the overlay output has stalled and repeated swapchain recreates
// failed to recover (e.g. SurfaceFlinger dropped the overlay during an HDR
// color-mode switch). The Java service polls this to end the session
// gracefully instead of spinning until the vendor lmkd kills the process.
