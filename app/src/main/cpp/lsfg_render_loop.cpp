...
struct State {
    ...
    struct SwapchainState {
        uint32_t acquireTimeoutStreak = 0;  // consecutive VK_TIMEOUT acquires
        // Timeout-dominated episode timer. SurfaceFlinger can release a single
        // buffer mid-stall, so a *consecutive* counter gets spuriously reset —
        // the episode timer keeps running until a sustained recovery.
        bool stallEpisode = false;
        State::Clock::time_point stallEpisodeStart{};
    ...
    // Set when the swapchain keeps stalling even after recreates — Java watchdog ends the session.
    std::atomic<bool> outputStalled{false};
...
bool blitOutputToSwapchain(const AhbImage &src) {
    ...
    if (ar != VK_SUCCESS && ar != VK_SUBOPTIMAL_KHR) {
        ...
        // Sustained TIMEOUT means SurfaceFlinger stopped releasing our
        // buffers (observed during HDR video playback: the display color-mode
        // switch drops our SDR overlay from composition).  A fresh swapchain
        // re-kicks the compositor; without this the overlay stays frozen.
        g.swap.acquireTimeoutStreak++;
        g.swap.successStreak = 0;
        const auto stallNow = State::Clock::now();
        if (!g.swap.stallEpisode) {
            g.swap.stallEpisode = true;
            g.swap.stallEpisodeStart = stallNow;
        }
        const float episodeSec =
            std::chrono::duration_cast<std::chrono::duration<float>>(
                stallNow - g.swap.stallEpisodeStart).count();
        if (g.swap.acquireTimeoutStreak >= 4) {
            LOGW("swapchain stalled after %u acquire timeouts - forcing recreate (episode %.1fs)",
                 g.swap.acquireTimeoutStreak, episodeSec);
            g.swap.acquireTimeoutStreak = 0;
            g.swap.outOfDate = true;
        }
        if (episodeSec >= 5.0f) {
            // Still timeout-dominated after 5s despite recreates: SurfaceFlinger
            // has dropped the overlay from composition for good (observed during
            // HDR video playback when the display switches color modes). Spinning
            // here would freeze the screen and eventually get the process killed
            // by the vendor lmkd (osvelte_lmkd, adj 50). Raise the stalled flag so
            // the Java-side watchdog can end the session gracefully instead.
            if (!g.outputStalled.exchange(true, std::memory_order_relaxed)) {
                LOGW("output stalled for %.1fs despite recreates - requesting session end",
                     episodeSec);
            }
        }
        return false;
    ...
    g.swap.acquireTimeoutStreak = 0;
    // Only a sustained run of successful acquires counts as recovery —
    // SurfaceFlinger can release a single buffer mid-stall. 30 consecutive
    // successes (~0.5s at 60fps) close the stall episode.
    if (++g.swap.successStreak >= 30 && g.swap.stallEpisode) {
        g.swap.stallEpisode = false;
        LOGI("swapchain recovered after stall episode");
    }
    g.outputStalled.store(false, std::memory_order_relaxed);
...
bool initFramegen(const char *cacheDir) {
    ...
    // Persist the Vulkan pipeline cache across sessions: after the first
    // session compiles the compute pipelines, rotation restarts and later
    // sessions load the driver's binary cache and skip the ~2 s recompile.
    if (!cache.empty()) {
        char pcName[96];
        std::snprintf(pcName, sizeof(pcName), "/framegen_pcache_%016llx.bin",
                      (unsigned long long)g.vk.deviceUuid);
        LSFG::Core::set_pipeline_cache_path(cache + pcName);
    }
...
void blitOutputToWindow(const AhbImage &out, bool allowGpuPost = true) {
    ...
    // CPU blit path runs on the worker thread while setOutputSurface() may
    // concurrently release/replace g.outWindow (holding g.mu).  Without the
    // lock below, ANativeWindow_setBuffersGeometry/_lock/_unlockAndPost can
    // touch a window that was just released -> SIGSEGV (observed on the
    // JNISurfaceTextu thread, 2026-09-07 18:13).
    std::lock_guard<std::mutex> cpuBlitLock(g.mu);
    if (g.outWindow == nullptr) return;  // detached between entry check and lock
...
void workerThread() {
    ...
    while (true) {
        State::PendingFrame pendingFrame{};
        {
            ...
            if (g.stopRequested) {
                // Fast shutdown: drop the whole pending queue instead of
                // processing it. Each queued frame would otherwise block in a
                // ~500 ms vkAcquireNextImageKHR timeout on a dead swapchain
                // (observed as a ~3 s tail on every rotation restart).
                for (auto &pf : g.pending) {
                    if (pf.ahb != nullptr) AHardwareBuffer_release(pf.ahb);
                }
                g.pending.clear();
                return;
            }
...
bool isOutputStalled() {
    return g.outputStalled.load(std::memory_order_relaxed);
}
