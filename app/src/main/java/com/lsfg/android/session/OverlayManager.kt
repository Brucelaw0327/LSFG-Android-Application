...
class OverlayManager(private val ctx: Context) {
    ...
    private var windowHiddenOffscreen = false
    ...
    fun bringToFront() {
        ...
        runCatching { wm.updateViewLayout(r, lp) }
            .onSuccess { reassertEmptyTouchableRegion(r) }
            .onFailure { LsfgLog.w(TAG, "bringToFront updateViewLayout failed", it) }
    }

    /**
     * WMS may reset the surface insets state on relayout, silently restoring a
     * full-screen touchable region and eating every touch on the display (the
     * "stuck overlay locks the whole screen" failure). Re-assert the empty
     * region after every relayout — cheap, idempotent, self-healing.
     */
    private fun reassertEmptyTouchableRegion(host: View) {
        runCatching {
            val rootSc = host.rootSurfaceControl ?: return
        ...
        }.onFailure { Log.w(TAG, "reassert setTouchableRegion(empty) failed", it) }
    }
    ...
    fun updateFps(capturedFps: Float, postedFps: Float) {
        ...
        val text = ctx.getString(
            R.string.hud_fps_stats,
            "%.1f".format(capturedFps), "%.1f".format(postedFps),
            "%.1f".format(latencyMs), "%.1f".format(queueMs),
        )
        v.post { v.text = text }
    }
    ...
    fun updateBenchmarkProgress(label: String, elapsedMs: Long, totalMs: Long) {
        ...
        val text = ctx.getString(R.string.bench_remaining, label, remainingSec)
        l.post {
            l.text = text
    ...
    /**
     * Visually hides the overlay WITHOUT tearing down the render pipeline:
     * the whole window is pushed off-screen (FLAG_LAYOUT_NO_LIMITS is
     * already set at show() time), keeping the TextureView SurfaceTexture
     * alive so returning to the target app is instant - no surface
     * destroy/re-attach. Used by the "overlay only inside the target app"
     * behaviour.
     */
    fun setWindowHidden(hidden: Boolean) {
        if (windowHiddenOffscreen == hidden) return
        val r = root ?: return
        windowHiddenOffscreen = hidden
        r.post {
            val lp = r.layoutParams as? WindowManager.LayoutParams ?: return@post
            lp.x = if (hidden) -overlayWidth else 0
            runCatching { hostWindowManager?.updateViewLayout(r, lp) }
                .onSuccess { reassertEmptyTouchableRegion(r) }
                .onFailure { Log.w(TAG, "setWindowHidden updateViewLayout failed", it) }
        }
    }
    ...
    fun hide() {
        val r = root
        if (r == null) {
            LsfgLog.w(TAG, "hide(): root already null — if a view was attached it now LEAKS")
            return
        }
        val wm = hostWindowManager
        if (wm == null) {
            LsfgLog.w(TAG, "hide(): hostWindowManager null — window may LEAK")
        } else {
            var removed = false
            runCatching { wm.removeView(r) }
                .onSuccess { removed = true }
                .onFailure { LsfgLog.w(TAG, "overlay removeView FAILED", it) }
            if (removed) {
                LsfgLog.i(TAG, "overlay window removed")
            } else if (r.isAttachedToWindow) {
                // A leaked overlay whose touchable region gets reset to the full
                // window frame by any later WMS relayout locks the entire screen
                // (2026-09-07 19:18 lockup). Retry removal on the main looper and
                // keep the touch region empty between attempts.
                ...
                val retry = object : Runnable {
                    override fun run() {
                        ...
                        if (!r.isAttachedToWindow) {
                            LsfgLog.i(TAG, "overlay detached on remove-retry #$attempt")
                            return
                        }
                        reassertEmptyTouchableRegion(r)
                        runCatching { wm.removeView(r) }
                            .onSuccess { LsfgLog.i(TAG, "overlay window removed on retry #$attempt") }
                            .onFailure {
                                LsfgLog.w(TAG, "overlay removeView retry #$attempt FAILED", it)
                                if (attempt < 10) handler.postDelayed(this, 200L)
                                else LsfgLog.e(TAG, "overlay window LEAKED after 10 retries — touch region kept empty")
                            }
                    }
                }
                handler.postDelayed(retry, 200L)
            } else {
                LsfgLog.w(TAG, "overlay removeView FAILED and view already detached")
            }
        }
    ...
    private fun installEmptyTouchableRegion(host: View): Any? {
        val applyEmptyRegion: () -> Unit = {
            runCatching {
                val rootSc = host.rootSurfaceControl
                if (rootSc == null) {
    ...
    private fun requestMaxRefreshRate(surface: Surface) {
        // 3-arg setFrameRate + CHANGE_FRAME_RATE_ALWAYS require API 31 (S).
        if (!surface.isValid || requestedRefreshRateHz <= 0f) {
            return
        }
        runCatching {
    ...
    private fun syncOverlayGeometry() {
        ...
        if (lp != null) {
            lp.width = newW
            lp.height = newH
            // A window parked off-screen for the rotation restart must
            // move with the new geometry: the offset was computed against
            // the pre-rotation width, and without re-applying it the stale
            // frozen frame covers part of the post-rotation screen.
            if (windowHiddenOffscreen) lp.x = -newW
            runCatching { wm.updateViewLayout(r, lp) }
                .onSuccess { reassertEmptyTouchableRegion(r) }
