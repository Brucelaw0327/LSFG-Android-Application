...
object LauncherSheetView {
    ...
    fun build(
        ...
        // Refresh: re-probe the foreground app; the header updates in place.
        // Fixes the panel occasionally showing the launcher while a game is
        // actually in front (probe raced the app switch).
        if (onRefresh != null) {
            val refreshBg = GradientDrawable().apply {
                shape = GradientDrawable.RECTANGLE
                cornerRadius = 14f * density
                setColor(0x00000000)
                setStroke((1 * density).toInt(), COLOR_PANEL_STROKE)
            }
            val refresh = Button(ctx).apply {
                text = ctx.getString(R.string.auto_overlay_sheet_refresh)
                setTextColor(COLOR_PRIMARY)
        ...
        // Secondary action: disable for this app (hidden in always-show mode).
        if (onDisableForApp != null) {
        ...
        val disable = Button(ctx).apply {
            ...
            lp.topMargin = (10 * density).toInt()
            layoutParams = lp
            setOnClickListener { onDisableForApp?.invoke() }
        }
