...
class SettingsDrawerOverlay(
    ...
    @Volatile private var expanded: Boolean = false
    ...
    /** True while the full panel is expanded — touches on it belong to our
     *  overlay UI and must never trigger the touch-pause parking path. */
    ...
    fun show() {
        ...
        val (initW, initH) = realScreenSize()
        screenW = initW
        screenH = initH
        ...
        LsfgLog.i(TAG, "drawer show (added)")
    }

    /** Raise the drawer window above the framegen overlay after the overlay
     *  re-fronted itself (touch-pause restore / target-foreground show). */
    fun bringToFront() {
        val r = root ?: return
        ...
        runCatching {
            wm.removeView(r)
            wm.addView(r, lp)
        }.onFailure { Log.w(TAG, "drawer bringToFront failed", it) }
        LsfgLog.i(TAG, "drawer bringToFront")
    }

    fun hide() {
        val r = root ?: return
        val wm = hostWindowManager
        LsfgLog.i(TAG, "drawer hide")
        ...
        if (wm != null) {
            ...
            runCatching { wm.removeView(r) }
                ...
                .onFailure { LsfgLog.w(TAG, "drawer removeView FAILED", it) }
            if (removed) {
                LsfgLog.i(TAG, "drawer window removed")
            } else if (r.isAttachedToWindow) {
                // Retry on the main looper — a leaked expanded drawer window
                // (fullscreen scrim) eats every touch on screen.
                ...
                val retry = object : Runnable {
                    override fun run() {
                        ...
                        if (!r.isAttachedToWindow) return
                        runCatching { wm.removeView(r) }
                            .onSuccess { LsfgLog.i(TAG, "drawer window removed on retry #$attempt") }
                            .onFailure {
                                LsfgLog.w(TAG, "drawer removeView retry #$attempt FAILED", it)
                                if (attempt < 10) handler.postDelayed(this, 200L)
                            }
                    }
                }
    ...
    private fun buildPanel(): View {
        ...
        val frameGenSection = collapsibleSection(panel, ctx.getString(R.string.drawer_section_framegen), initiallyExpanded = true)

        frameGenSection.addView(switchRow(
            label = ctx.getString(R.string.drawer_label_lsfg_fg),
        ...
        frameGenSection.addView(switchRow(
            label = ctx.getString(R.string.drawer_label_perf_mode),
        ...
        frameGenSection.addView(switchRow(
            label = ctx.getString(R.string.drawer_label_antialias),
        ...
        if (fp16Available) {
            frameGenSection.addView(switchRow(
                label = ctx.getString(R.string.drawer_label_fp16),
        ...
        frameGenSection.addView(
            sliderRow(
                labelText = ctx.getString(R.string.drawer_label_mult),
        ...
        frameGenSection.addView(
            sliderRow(
                labelText = ctx.getString(R.string.drawer_label_flow_scale),
                valueView = flowValue,
            ),
        )
        frameGenSection.addView(SeekBar(ctx).apply {
        ...
        val pacingSection = collapsibleSection(panel, ctx.getString(R.string.drawer_section_pacing))
        ...
        val hudSection = collapsibleSection(panel, ctx.getString(R.string.drawer_section_hud))
        hudSection.addView(switchRow(
            label = ctx.getString(R.string.drawer_label_fps),
        ...
        hudSection.addView(switchRow(
            label = ctx.getString(R.string.drawer_label_graph),
            initial = initialFrameGraph,
        ) {
            prefs.setFrameGraphEnabled(it)
            frameGraphListener?.onFrameGraphChanged(it)
        })
        hudSection.addView(miniHeader(ctx.getString(R.string.drawer_handle)))
        ...
        val stopBtn = Button(ctx).apply {
            text = ctx.getString(R.string.drawer_end_session)
    ...
    private fun setProgress(p: Float) {
        ...
        // 自愈：面板可见的任何时刻强制确保窗口处于全屏展开态（幂等），
        // 杜绝"面板渲染在收缩窗口里被裁剪"。
        if (p > 0f) expandWindow()
    ...
    /** 真实窗口尺寸：API 30+ 用 currentWindowMetrics，失败回退 displayMetrics。 */
    ...
    /**
     * expandWindow() 的 resize 是异步 binder 调用。若立刻开滑动动画，面板可能在旧
     * 的小窗口内渲染并被永久裁剪（"抽屉显示一半" bug）。这里轮询 root 实际布局尺寸，
     * 等窗口接近全屏后再动画，220ms 兜底。
     */
    private fun waitForPanelWindowThenAnimate(target: Float) {
        val r = root
        if (r == null || !r.isAttachedToWindow) {
            animateTo(target)
            return
        }
        val minW = (screenW * 0.85f).toInt()
        val minH = (screenH * 0.85f).toInt()
        var waited = 0L
        fun poll() {
            if (r.width >= minW && r.height >= minH || waited >= 220L) {
                LsfgLog.i(
                    TAG,
                    "panel animate start (full=" + (r.width >= minW && r.height >= minH) +
                        ", waitMs=" + waited + ")",
                )
                animateTo(target)
            } else {
                waited += 16L
                r.postDelayed({ poll() }, 16L)
            }
        }
        poll()
    }
    ...
    private fun relayoutForCurrentDisplay() {
        ...
        // 上面的 relayout 已取消 settleAnimator：若抽屉当时处于打开中，progress 会
        // 冻结在中间值（面板被平移一半卡住）。重新展开并收敛到全开。
        if (progress > 0f) {
            expandWindow()
            waitForPanelWindowThenAnimate(1f)
        }
    ...
    private fun buildPacingControls(
        ...
        val presetItems = listOf(
            PacingPreset.SMOOTH to ctx.getString(R.string.preset_smooth),
            PacingPreset.BALANCED to ctx.getString(R.string.preset_balanced),
            PacingPreset.LOW_LATENCY to ctx.getString(R.string.preset_low_latency),
            PacingPreset.CUSTOM to ctx.getString(R.string.preset_custom),
        )
        ...
        parent.addView(switchRow(
            label = ctx.getString(R.string.drawer_label_vsync),
        ...
        val fpsCapValue = TextView(ctx).apply {
            ...
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 13f)
            typeface = android.graphics.Typeface.create(typeface, android.graphics.Typeface.BOLD)
            text = if (initial.targetFpsCap <= 0) ctx.getString(R.string.drawer_unlimited) else "${initial.targetFpsCap} fps"
        }
        parent.addView(sliderRow(ctx.getString(R.string.drawer_label_fps_cap), fpsCapValue))
        parent.addView(SeekBar(ctx).apply {
            ...
            setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(seekBar: SeekBar?, p: Int, fromUser: Boolean) {
                    ...
                    fpsCapValue.text = if (v <= 0) ctx.getString(R.string.drawer_unlimited) else "${v} fps"
                    if (fromUser) {
                        pendingFpsCap = v
        ...
        val advanced = collapsibleSection(parent, ctx.getString(R.string.drawer_section_advanced), initiallyExpanded = false)
        advanced.addView(TextView(ctx).apply {
            text = ctx.getString(R.string.drawer_advanced_note)
            setTextColor(COLOR_ON_SURFACE)
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 11f)
            alpha = 0.7f
            setPadding(0, dp(4), 0, dp(8))
        ...
        advanced.addView(sliderRow(ctx.getString(R.string.drawer_label_ema), emaValue))
        ...
        advanced.addView(sliderRow(ctx.getString(R.string.drawer_label_outlier), outlierValue))
        attachAdvancedSlider(
        ...
        advanced.addView(sliderRow(ctx.getString(R.string.drawer_label_slack), slackValue))
        attachAdvancedSlider(
        ...
        advanced.addView(sliderRow(ctx.getString(R.string.drawer_label_queue), queueValue))
        attachAdvancedSlider(
        ...
        advanced.addView(Button(ctx).apply {
            text = ctx.getString(R.string.drawer_reset)
            isAllCaps = false
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 13f)
    ...
    private fun buildGpuImageQualitySection(
        ...
        val gpuSection = collapsibleSection(panel, ctx.getString(R.string.drawer_section_gpu))
        gpuSection.addView(switchRow(
            label = ctx.getString(R.string.drawer_label_gpu_post),
        ...
        val gpuAdvanced = collapsibleSection(gpuSection, ctx.getString(R.string.drawer_section_advanced), initiallyExpanded = false)
    ...
    private fun buildNpuImageQualitySection(
        ...
        val npuSection = collapsibleSection(panel, ctx.getString(R.string.drawer_section_npu))
    ...
    private fun buildCpuImageQualitySection(
        ...
        val cpuSection = collapsibleSection(panel, ctx.getString(R.string.drawer_section_cpu))
        cpuSection.addView(switchRow(
            label = ctx.getString(R.string.drawer_label_cpu_post),
    ...
    private fun drawerEdgeChipRow(
        ...
        val items = listOf(
            DrawerEdge.LEFT to ctx.getString(R.string.drawer_edge_left),
            DrawerEdge.RIGHT to ctx.getString(R.string.drawer_edge_right),
            DrawerEdge.TOP to ctx.getString(R.string.drawer_edge_top),
            DrawerEdge.BOTTOM to ctx.getString(R.string.drawer_edge_bottom),
        )
            ...
            when (ev.actionMasked) {
                ...
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                    if (iconDragMoved) {
                        ...
                        // Tap — open the panel.
                        if (!expanded) {
                            LsfgLog.i(TAG, "entry tapped - expanding panel")
                            expandWindow()
                            waitForPanelWindowThenAnimate(1f)
                        }
                    }
    ...
    companion object {
        /** ElapsedRealtime of the last touch on our own overlay UI (drawer strip,
         *  launcher dot). The touch-pause parking logic checks this so touches on
         *  the overlay chrome never park framegen or hide the drawer mid-tap —
         *  hiding the window (removeView) swallowed clicks like End session. */
        @Volatile private var lastUiTouchAtMs: Long = 0L

        fun markUiTouch() {
            lastUiTouchAtMs = android.os.SystemClock.elapsedRealtime()
        }

        fun uiTouchFresh(graceMs: Long): Boolean =
            android.os.SystemClock.elapsedRealtime() - lastUiTouchAtMs < graceMs
