...
class ProjectionRequestActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Privileged capture sources need no MediaProjection consent or app
        // picker — start the service directly with the matching intent.
        when (prefs.captureSource) {
            ...
            CaptureSource.ROOT -> {
                val intent = LsfgForegroundService.buildRootStartIntent(
                    ctx = this,
                    targetPackage = targetPackage,
                    fpsCounter = prefs.fpsCounterEnabled,
                )
